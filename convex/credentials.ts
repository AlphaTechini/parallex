...
export const getZhipuCredentialStatus = query({
  args: {},
  handler: async (ctx) => {
    const ownerId = await getAuthenticatedUserId(ctx);
    const credential = await ctx.db
...
export const upsertZhipuCredential = mutation({
  args: { apiKey: v.string() },
  handler: async (ctx, args) => {
    const ownerId = await getAuthenticatedUserId(ctx);
    const apiKey = args.apiKey.trim();
    if (!apiKey) {
      throw new Error("INVALID_ZHIPU_KEY");
    }
    ...
    if (existing === null) {
      await ctx.db.insert("zhipuCredentials", {
        ownerId,
        ciphertext: encrypted.ciphertextB64,
        initializationVector: encrypted.ivB64,
        keyVersion: 1,
      ...
      await ctx.db.patch("zhipuCredentials", existing._id, {
        ciphertext: encrypted.ciphertextB64,
        initializationVector: encrypted.ivB64,
        authenticationTag: undefined,
        keyVersion: 1,
...
export const deleteZhipuCredential = mutation({
  args: {},
  handler: async (ctx) => {
    const ownerId = await getAuthenticatedUserId(ctx);
    const credential = await ctx.db
    ...
    const now = Date.now();
    await ctx.db.patch("zhipuCredentials", credential._id, {
      ciphertext: "",
      initializationVector: "",
      authenticationTag: undefined,
      status: "deleted",
...
export const getProviderCredentialStatuses = query({
  ...
  handler: async (ctx) => {
    const ownerId = await getAuthenticatedUserId(ctx);
    const [openai, zhipu] = await Promise.all([
      ctx.db
        .query("openaiCredentials")
        .withIndex("by_owner", (q) => q.eq("ownerId", ownerId))
        .unique(),
