...
This directory contains the server-side execution layer: the resumable research run worker and its state mutations, the OpenAI stream consumer, the tool dispatch action, the AgentMail email sender, and the inbox provisioner. Workers have no browser session; they load owned records, revalidate state, and only then touch provider APIs or secrets.
