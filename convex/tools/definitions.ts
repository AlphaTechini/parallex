...
const TOOL_DEFINITIONS: readonly FunctionTool[] = [
  ...
  tool(
    "create_template_draft",
    "Create a private structured bot-template draft for the current user to review, edit, and deploy in the browser. Use after the user asks for a reusable specialist or agrees that a repeatable workflow should become a template. This function creates a draft only and can never deploy a bot.",
    objectSchema({
      name: stringSchema({ minLength: 3, maxLength: 80 }),
      shortDescription: stringSchema({ minLength: 12, maxLength: 240 }),
      mission: stringSchema({ minLength: 20, maxLength: 500 }),
      outcome: stringSchema({ minLength: 12, maxLength: 500 }),
      exampleRequest: stringSchema({ minLength: 8, maxLength: 500 }),
      category: {
        type: "string",
        enum: ["everyday", "software", "ai", "hardware", "custom"],
      },
      role: stringSchema({ minLength: 20, maxLength: 1500 }),
      intendedUser: stringSchema({ minLength: 12, maxLength: 1000 }),
      intakeQuestions: stringArray({ minItems: 3, maxItems: 12 }),
      hardConstraints: stringArray({ minItems: 1, maxItems: 12 }),
      evaluationCriteria: {
        type: "array",
        minItems: 3,
        maxItems: 12,
        items: objectSchema({
          name: stringSchema({ minLength: 2, maxLength: 120 }),
          description: stringSchema({ minLength: 8, maxLength: 800 }),
          importance: {
            type: "string",
            enum: ["essential", "high", "medium", "low"],
          },
          weight: { type: "integer", minimum: 1, maximum: 100 },
          priceRelevant: { type: "boolean" },
        }),
      },
      pricingMethod: stringSchema({ minLength: 30, maxLength: 3000 }),
      researchWorkflow: stringArray({ minItems: 3, maxItems: 15 }),
      sourceStandards: stringArray({ minItems: 2, maxItems: 12 }),
      rankingMethod: stringArray({ minItems: 2, maxItems: 12 }),
      outputRequirements: stringArray({ minItems: 3, maxItems: 15 }),
      actionRules: stringArray({ minItems: 1, maxItems: 12 }),
      uncertaintyRules: stringArray({ minItems: 2, maxItems: 12 }),
      safetyBoundaries: stringArray({ minItems: 1, maxItems: 12 }),
      schedules: {
        type: "array",
        maxItems: 3,
        items: objectSchema({
          id: stringSchema({ minLength: 2, maxLength: 80 }),
          name: stringSchema({ minLength: 3, maxLength: 160 }),
          summary: stringSchema({ minLength: 8, maxLength: 500 }),
          researchPrompt: stringSchema({ minLength: 20, maxLength: 6000 }),
          frequency: {
            type: "string",
            enum: ["hourly", "daily", "weekly", "monthly"],
          },
          ...
          localMinute: { type: "integer", minimum: 0, maximum: 59 },
          weekday: nullable({ type: "integer", minimum: 1, maximum: 7 }),
          dayOfMonth: nullable({ type: "integer", minimum: 1, maximum: 31 }),
        }),
      },
