...
This directory contains the Convex Auth wiring, static-site hosting, application schema, OpenAI research worker, Firecrawl provider integration, AgentMail delivery and webhook handling, report persistence, and schedule execution.
...
- Long work is split across short Node actions with lease and generation counters plus a delayed watchdog, because a single action cannot outlast the Convex execution limit. OpenAI checkpoints stream cursors so the run can survive interruptions and browser closure, but the state machine has more explicit checkpoints than a naive implementation.
