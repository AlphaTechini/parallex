...
To find the paginated chat list for one bot visit [ChatList.tsx](file:///C:/Hackathons/Parallex/src/components/bots/ChatList.tsx). Chat creation sits in the top right of the conversations panel through [NewChatButton.tsx](file:///C:/Hackathons/Parallex/src/components/chat/NewChatButton.tsx).

To find the settings tab panel with read-only bot information, the archive action, and the memory editor visit [BotSettingsPanel.tsx](file:///C:/Hackathons/Parallex/src/components/bots/BotSettingsPanel.tsx).

To find the client detail view that reads `botId` from the static route query string and switches between the Chat, Schedules, and Settings tabs visit [BotDetailPageContent.tsx](file:///C:/Hackathons/Parallex/src/components/bots/BotDetailPageContent.tsx).
