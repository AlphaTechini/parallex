...
- The key form is write-only from the client's perspective: the query surface returns configured state and a display hint, never the key, so no client path can leak the secret.
